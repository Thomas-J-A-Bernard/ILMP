...
def NeutronsLowE(h, Rc, s, w):
    '''
    DESCRIPTIONS:
        reduced version that does not do cross-sections but includes thermal and
        epithermal spectra Sato et al. (2008) Neutron Spectrum Analytical Function Approximation (PARMA)
    ----------
    PARAMETERS:
    h : float
    ...
    RETURNS
    site : structure
        low neutron spectrum
    '''
    
    # convert pressure (hPa) to atm depth (g/cm2)
    x = h*1.019716
    E = np.logspace(-8, 0, num=200)
    ...
    PhiB = np.zeros(len(Rc))
    PhiG = np.zeros(len(Rc))
    PhiGMev = np.zeros(len(Rc))
    ethflux = np.zeros(len(Rc))
    thflux = np.zeros(len(Rc))
    ...
    for a in range(0, len(Rc)):
        ...
        clipindexethmax = np.where(E <= 1e-1)[0][-1]
        clipindexethmin = np.where(E >= 5e-7)[0][0]
        ethflux[a] =  -np.trapz(E[clipindexethmin:clipindexethmax], x=PhiGMev[clipindexethmin:clipindexethmax])
        clipindexth = np.where(E <= 5e-7)[0][-1]
        thflux[a] = -np.trapz(E[0:clipindexth], x=PhiGMev[0:clipindexth])

...
def Muons(h, Rc, s):
    '''
    DESCRIPTIONS:
        Sato et al. (2008) Muon Spectrum analytical Function Approximation (PARMA)
    ----------
    PARAMETERS:
    h : float
    ...
    x = h*1.019716
    E = np.logspace(1, 5.9030, num=200)
    ...
    mflux = MFlux()
    mflux.total = np.zeros(len(Rc))
    mflux.neg = np.zeros((len(Rc), len(E)))
    mflux.pos = np.zeros((len(Rc), len(E)))
    mflux.nint = np.zeros(len(Rc))
    mflux.pint = np.zeros(len(Rc))
    for a in range(len(Rc)):
        ...
        mflux.total[a] = -np.trapz(E[clipindex:], x=Phimu[a,clipindex:])
        # Differential fluxes for negative and positive muons
        mflux.neg[a,:] = phimun
        mflux.pos[a,:] = phimup
        # Integral fluxes for positive and negative muons
        mflux.nint[a] = -np.trapz(E[clipindex:], x=phimun[clipindex:]);
        mflux.pint[a] = -np.trapz(E[clipindex:], x=phimup[clipindex:]);
        mflux.E = E;
        mflux.p = p;

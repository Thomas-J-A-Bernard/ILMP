...
def Neutrons(h, Rc, s, w, consts, nuclide):
    '''
    DESCRIPTIONS:
    ...
    # convert pressure (hPa) to atm depth (g/cm2)
    x = h*1.019716
    ...
    # thermal neutron energy in MeV
    Et = 2.5e-8
    
    # integrated neutron flux < 15 MeV
    smin = 400      # unit in MV
    smax = 1200     # unit in MV
    ...
    PhiB = np.zeros(len(E))
    PhiG = np.zeros(len(E))
    PhiGMev = np.zeros(len(E))
    p10n = np.zeros(len(E))
    p14n = np.zeros(len(E))
    p26n = np.zeros(len(E))
    p3n = np.zeros(len(E))
    p36Can = np.zeros(len(E))
    p36Kn = np.zeros(len(E))
    p36Tin = np.zeros(len(E))
    p36Fen = np.zeros(len(E))
    
    n = N()
    n.P3n = np.zeros(len(Rc))
    n.P10n = np.zeros(len(Rc))
    n.P14n = np.zeros(len(Rc))
    n.P26n = np.zeros(len(Rc))
    n.nflux = np.zeros(len(Rc))
    for a in range(0, len(Rc)):
        ...
        PhiB = (c1*(E/c2)**c3)*np.exp(-E/c2) + c4*np.exp((-(np.log10(E) - np.log10(c5))**2)/(2*(np.log10(c6))**2)) + c7*np.log10(E/c8)*(1 + np.tanh(c9*np.log10(E/c10)))*(1 - np.tanh(c11*np.log10(E/c12)));
        
        PhiG = PhiL*(PhiB*fG + PhiT)
        PhiGMev = PhiG/E
        
        # Make sure the clip index is consistent with the definition of E above
        clipindex = np.where(E <= 1)[0][-1]
        
        if nuclide == 3:
            n.P3n[a] = -(np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['OnxHe3T'][clipindex:]) + np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['SinxHe3T'][clipindex:]/2))*consts['Natoms3']*1e-27*3.1536e7   
        elif nuclide == 10:  
            n.P10n[a] = -(np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['O16nxBe10'][clipindex:]) + np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['SinxBe10'][clipindex:]/2))*consts['Natoms10']*1e-27*3.1536e7   
        elif nuclide == 14:
            n.P14n[a] = -(np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['O16nn2pC14'][clipindex:])+ np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['SinxC14'][clipindex:]/2))*consts['Natoms14']*1e-27*3.1536e7   
        elif nuclide == 26:
            n.P26n[a] = -np.trapz(E[clipindex:], x=PhiGMev[clipindex:]*consts['SinxAl26'][clipindex:])*consts['Natoms26']*1e-27*3.1536e7
        else:
            n.nflux[a] = -np.trapz(E[clipindex:], x=PhiGMev[clipindex:]) 

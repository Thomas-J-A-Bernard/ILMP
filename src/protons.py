...
def Protons(h, Rc, s, consts, nuclide):
    '''
    DESCRIPTIONS:
        Sato et al. (2008) Protons Spectrum analytical Function Approximation (PARMA)
    ----------
    PARAMETERS:
    h : float
    ...
    Beta = np.zeros(len(E))
    Rlis = np.zeros(len(E))
    phiTOA = np.zeros(len(E))
    phiLIS = np.zeros(len(E))
    phiSec = np.zeros(len(E))
    phiPtot = np.zeros(len(E))
    p10p = np.zeros(len(E))
    ...
    p = P()
    p.P3p = np.zeros(len(Rc))
    p.P10p = np.zeros(len(Rc))
    p.P14p = np.zeros(len(Rc))
    p.P26p = np.zeros(len(Rc))
    p.pflux = np.zeros(len(Rc))
    for a in range(0, len(Rc)):
        ...
        Es = a13*(Ec - a14*x);
        Es1 = np.maximum(a15, Es);
        Es2 = np.maximum(a16, Es);
        ...
        if nuclide == 3:
            p.P3p[a] = -(np.trapz(E, x=phiPtot*consts['OpxHe3T']) + np.trapz(E, x=phiPtot*consts['SipxHe3T']/2))*consts['Natoms3']*1e-27*3.1536e7;    
        elif nuclide == 10:
            p.P10p[a] = -(np.trapz(E, x=phiPtot*consts['O16pxBe10']) + np.trapz(E, x=phiPtot*consts['SipxBe10']/2))*consts['Natoms10']*1e-27*3.1536e7;
        elif nuclide == 14: 
            p.P14p[a] = -(np.trapz(E, x=phiPtot*consts['O16pxC14'])+ np.trapz(E, x=phiPtot*consts['SipxC14']/2))*consts['Natoms14']*1e-27*3.1536e7;
        elif nuclide == 26:
            p.P26p[a] = -np.trapz(E, x=phiPtot*consts['SipxAl26'])*consts['Natoms26']*1e-27*3.1536e7 
        else:
            p.pflux[a] = -np.trapz(E[clipindex:], x=phiPtot[clipindex:]);
